...
public class JwtAccessTokenCustomizer extends DefaultAccessTokenConverter implements JwtAccessTokenConverterConfigurer {
    ...
    public OAuth2Authentication extractAuthentication(Map<String, ?> tokenMap) {
        ...
        Set<String> groupsList = extractGroups(token); // extracting client names
        List<GrantedAuthority> authorities = extractRoles(token); // extracting client roles

        OAuth2Authentication authentication = super.extractAuthentication(tokenMap);
        OAuth2Request oAuth2Request = authentication.getOAuth2Request();

        accessControl.getUserGroupsMap().put(authentication.getPrincipal().toString(),new ArrayList<>(groupsList));
    ...
    private Set<String> extractGroups(JsonNode jwt) {
        LOG.debug("Begin extractClients: jwt = {}", jwt);
        if (jwt.has("usergroup")) {
            JsonNode resourceAccessJsonNode = jwt.path("usergroup");
            final Set<String> groupsNames = new HashSet<>();
            resourceAccessJsonNode.elements()
                    .forEachRemaining(g -> groupsNames.add(g.toString().replaceAll("\"|/","")));

            LOG.debug("End extractClients: clients = {}", groupsNames);
            return groupsNames;
